...
export default async function page() {
  ...
  const renderProject = (project: any, index: number, isCenter: boolean = false) => (
    <div key={index} className={`flex flex-col items-start p-0 gap-[32px] ${isCenter ? 'max-w-[348px] sm:max-w-full sm:min-w-full min-h-[630px] sm:min-h-[760px]' : 'max-w-[348px] sm:max-w-full md:max-w-[43vw] min-h-[444px]'}`}>
      <Image src={project.image} alt={project.title} width={500} height={500} className={`w-full ${isCenter ? 'max-h-[556px]' : 'h-[330px]'}`} />
      <div className="flex flex-col items-start p-0 gap-[24px] w-full min-h-[172px]">
        <div className="flex flex-row justify-between items-start p-0 gap-[16px] w-full min-h-[32px]">
          <h1 className={`font-semibold text-start ${isCenter ? 'max-w-[1176px] text-[18px] sm:text-[24px]' : 'text-[18px] sm:text-[24px]'} leading-[32px] text-[#1A1A1A] dark:text-white`}>
            {project.title}
          </h1>
        ...
        <p className="w-full min-h-[48px] font-normal text-start text-[13px] sm:text-[16px] leading-[24px] text-[#667085]">
          {project.description.slice(0, isCenter ? 150 : 100) + "..."}
        </p>
      </div>
      ...
      <div className="flex flex-col items-center px-0 py-[30px] gap-[64px] w-full min-h-[2080px] ">
        <div className='flex flex-col items-center px-4 sm:px-0 py-0 gap-[32px] w-[88.889vw] min-h-[2020px]'>
          ...
          <div className="flex flex-col md:flex-row items-start p-0 gap-[32px]">
            {data.filter((_: any, index: number) => index <= 1).map((project: any, index: number) => renderProject(project, index))}
          </div>
          {/* center */}
          <div className='w-full flex justify-center'>
            {data.filter((_: any, index: number) => index === 2).map((project: any, index: number) => renderProject(project, index, true))}
          </div>
          {/* last */}
          <div className="flex flex-col md:flex-row items-start p-0 gap-[32px]">
            {data.filter((_: any, index: number) => index >= 3).map((project: any, index: number) => renderProject(project, index))}
          </div>
        </div>
      </div>
    </div>
